(function( es, acx, raphael ) {
	...
	es.ui.AnyRequest = es.ui.Page.extend({
      ...
      window.setTimeout( function( query ) {
        ...
        es.json_editor = new JSONEditor( jQuery( '.json_editor' ).get( 0 ), {
          'indentation': 2,
          'search': false,
          'history': false,
          'change': function() {

            console.log( window.json_update_delay );

            if( window.json_update_delay ) {
              return;
            }

            window.json_update_delay = window.setTimeout( function() {

              jQuery( 'textarea[name="body"]' ).val( JSON.stringify( es.json_editor.get(), null, "  " ) );

              delete window.json_update_delay;
